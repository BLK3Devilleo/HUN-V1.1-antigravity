  ...
  if (!causes || causes.length === 0) {
    return (
      <motion.div
        ...
        className="col-span-full py-20 text-center bg-white border border-black/10 rounded-[28px] p-8 shadow-sm space-y-4 flex flex-col items-center justify-center"
      >
        <div className="w-16 h-16 rounded-2xl bg-[#F4F4F4] border border-black/5 flex items-center justify-center mx-auto shadow-sm">
          <Layers className="w-8 h-8 text-black/60" />
        </div>
    ...
    >
      {causes.map((cause) => (
        <motion.div
          ...
          className="group relative bg-white border border-black/10 rounded-[28px] p-4 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between"
        >
          <div className="space-y-3">
            {/* Container Multimedia */}
            <div className="relative w-full aspect-square bg-[#F4F4F4] rounded-[20px] overflow-hidden border border-black/5">
              {cause.media_url?.endsWith('.mp4') || cause.media_url?.endsWith('.mov') || cause.media_url?.endsWith('.webm') ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center gap-2 bg-black/5">
                  ...
                  <span className="text-[10px] font-bold text-black/40 uppercase tracking-wider">Sin imagen</span>
                </div>
              )}

              {/* Badge de Organización */}
              <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
                <span className="text-[10px] font-extrabold uppercase tracking-wider bg-black/85 text-white px-3 py-1 rounded-full backdrop-blur-md truncate max-w-[80%] shadow-sm flex items-center gap-1">
            ...
            {/* Info Text */}
            <div className="bg-[#F8F8F8] rounded-[18px] p-3.5 space-y-1 border border-black/5">
